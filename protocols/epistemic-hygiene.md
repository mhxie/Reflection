...
- **Tag taxonomy** lives here. The wiki schema (`wiki-schema.md`) defines the structure for files under `<paths.wiki>/`. The trust engine (Phase B, `scripts/trust.py`) reads only the `<paths.wiki>/` subtree.
